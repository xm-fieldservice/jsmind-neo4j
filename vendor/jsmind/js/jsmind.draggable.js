    ...
    jsMind.draggable.prototype = {
        ...
        _lookup_close_node:function(){
            ...
            for(var nodeid in nodes){
                var np,sp;
                node = nodes[nodeid];
                if(node.isroot || node.id == this.active_node.id){
                    continue;
                }
                ns = node.get_size();
                ...
                if(direct == jsMind.direction.right){
                    if(nl.x>(sx + sw)){
                        distance = Math.abs(nl.x - (sx + sw)) + Math.abs(nl.y + ns.h/2 - (sy + sh/2));
                        if(distance < min_distance){
                            min_distance = distance;
                            closest_node = node;
                            closest_p = {x:nl.x,y:nl.y+ns.h/2};
                            shadow_p = {x:sx+sw,y:sy+sh/2};
                        }
                    }
                }else{
                    if((nl.x+ns.w)<sx){
                        distance = Math.abs(nl.x + ns.w - sx) + Math.abs(nl.y + ns.h/2 - (sy + sh/2));
                        if(distance < min_distance){
                            min_distance = distance;
                            closest_node = node;
                            closest_p = {x:nl.x+ns.w,y:nl.y+ns.h/2};
                            shadow_p = {x:sx,y:sy+sh/2};
                        }
                    }
                }
            }
            if(!!closest_node){
                ...
                this.target_direct = direct;
                this._magnet_shadow({sp:shadow_p,np:closest_p});
            }else{
                this.target_node = null;
                this.target_direct = null;
                this.clear_lines();
        ...
        dragstart:function(e){
            ...
            var node_element = e.target || e.srcElement;
            var node = this.jm.get_node_from_element(node_element);
            if(!!node){
                this.active_node = node;
                this.offset_x = (e.clientX || e.touches[0].clientX) - node_element.offsetLeft;
                this.offset_y = (e.clientY || e.touches[0].clientY) - node_element.offsetTop;
                this.client_hw = node_element.clientWidth/2;
                this.client_hh = node_element.clientHeight/2;
                ...
                this.hlookup_delay = $w.setTimeout(function(){
                    this.hlookup_timer = $w.setInterval(function(){
                        this._lookup_close_node();
                    }.bind(this),options.lookup_interval);
                }.bind(this),options.lookup_delay);
                this.capture = true;
            }
        },
