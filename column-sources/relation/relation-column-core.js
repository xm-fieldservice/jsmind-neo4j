...
async function handleFileUpload() {
    ...
    try {
        ...
        if (file.name.endsWith('.json') || file.name.endsWith('.mindmap.json')) {
            ...
            // Markdown文档 - 上传到后端解析并导入Neo4j
            console.log('[文档上传] 开始上传Markdown文档:', file.name);
            const result = await uploadDocumentToNeo4j(file);
            console.log('[文档上传] 上传成功，结果:', result);
            alert(`✅ 文档导入成功！\n\n提取实体：\n- 项目: ${result.parsed.projects}个\n- 任务: ${result.parsed.tasks}个\n- 人员: ${result.parsed.persons}个\n- 关系: ${result.parsed.relationships}个\n\n已导入到Neo4j数据库！`);
        } else {
            throw new Error('不支持的文件类型，请上传.md/.json文件');
        }
        clearFileSelection();
...
// 上传Markdown文档到Neo4j
async function uploadDocumentToNeo4j(file) {
    try {
        const formData = new FormData();
        formData.append('file', file);
        
        console.log('[文档导入] 开始上传文档:', file.name);
        ...
        const result = await response.json();
        console.log('[文档导入] 导入成功:', result);
        
        return result;
    } catch (error) {
        console.error('[文档导入] 导入失败:', error);
        throw error;
    }
}
...
async function executeGeneratedCypher() {
    ...
    try {
        ...
        // 详细的错误日志
        console.error('[执行Cypher] 失败 - 错误类型:', error.constructor.name);
        console.error('[执行Cypher] 失败 - 错误消息:', error.message);
        console.error('[执行Cypher] 失败 - 完整错误:', error);
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError') || error.name === 'TypeError') {
            ...
            console.warn('[执行Cypher] 网络错误 - 后端服务未启动或无法访问');
        } else {
            errorMsg += '原因：' + error.message + '\n\n';
            errorMsg += '提示：可以复制Cypher到Neo4j Browser手动执行';
            
            console.error('[执行Cypher] 业务错误:', error.message);
        }
